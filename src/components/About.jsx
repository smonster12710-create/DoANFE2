import gsap from 'gsap';
import { SplitText } from 'gsap/all'
import { useGSAP } from '@gsap/react'

const About = () => {
	useGSAP(() => {
		const titleSplit = SplitText.create('#about h2', {
			type: 'words'
		})

		const scrollTimeline = gsap.timeline({
			scrollTrigger: {
				trigger: '#about',
				start: 'top center'
			}
		})

		scrollTimeline
			.from(titleSplit.words, {
				opacity: 0, duration: 1, yPercent: 100, ease: 'expo.out', stagger: 0.02
			})
			.from('.top-grid div, .bottom-grid div', {
				opacity: 0, duration: 1, ease: 'power1.inOut', stagger: 0.04,
			}, '-=0.5')
	})

	return (
		<div id="about">
			<div className="mb-16 md:px-0 px-5">
				<div className="content">
					<div className="md:col-span-8">
						<p className="badge">The Lore</p>
						<h2>
							Echoes of the ancient sky <span className="text-white">-</span>
							written in the stars
						</h2>
					</div>

					<div className="sub-content">
						<p>
							Long before our time, every constellation was a living myth whispered across generations. We bring those forgotten celestial chronicles back to life, turning the night sky into an immersive tapestry of timeless legends.
						</p>

						<div>
							<p className="md:text-3xl text-xl font-bold">
								<span>1000+</span> Years
							</p>
							<p className="text-sm text-white-100">
								Of cosmic myths and ancient stardust
							</p>
						</div>
					</div>
				</div>
			</div>

			<div className="top-grid">
				<div className="md:col-span-3">
					<div className="noisy" />
					<img src="/images/people-star.png" alt="grid-img-1" />
				</div>

				<div className="md:col-span-6">
					<div className="noisy" />
					<img src="/images/people-star2.png" alt="grid-img-2" />
				</div>

				<div className="md:col-span-3">
					<div className="noisy" />
					<img src="/images/people-star3.png" alt="grid-img-5" />
				</div>
			</div>

			<div className="bottom-grid">
				<div className="md:col-span-8">
					<div className="noisy" />
					<img src="/images/people-star4.png" alt="grid-img-3" />
				</div>

				<div className="md:col-span-4">
					<div className="noisy" />
					<img src="/images/people-star5.png" alt="grid-img-4" />
				</div>
			</div>

		</div>
	)
}
export default About
